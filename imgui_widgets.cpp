...
void ImGui::TextIcon(const char* icon, const char* text, ImU32 color) {

    PushStyleColor(ImGuiCol_Text, GetColorU32(color));

    PushFont(fonts::icon3); ImGui::Text(icon); PopFont();
    ImGui::SameLine(0, 5);
    ImGui::Text(text);
...
bool ImGui::Selector(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImStyle::selector::background_active : hovered ? ImStyle::selector::background_hov : ImStyle::selector::background, g.IO.DeltaTime * 6.f);
    it_anim->second.icon = ImLerp(it_anim->second.icon, selected ? ImStyle::general_color : ImStyle::selector::icon, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), ImStyle::button::rounding);

    RenderTextClipped(bb.Min + ImVec2(10, 0), bb.Max + ImVec2(10, 0), label, NULL, &label_size, ImVec2(0.0f, 0.5f), &bb);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.icon));
    PushFont(fonts::icon);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x - 40, 0), bb.Max + ImVec2(size_arg.x - 40, 0), "F", NULL, &label_size);
    PopFont();
    PopStyleColor();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImStyle::button::background_active : hovered ? ImStyle::button::background_hov : ImStyle::button::background, g.IO.DeltaTime * 6.f);
    it_anim->second.shadow_rect = ImLerp(it_anim->second.shadow_rect, IsItemActive() ? 65.f : 45.f, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), ImStyle::button::rounding);

    GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(10, 10), bb.Max - ImVec2(10, 10), GetColorU32(it_anim->second.background), it_anim->second.shadow_rect, ImVec2(0, 0), ImStyle::button::rounding);
...
bool ImGui::TextButton(const char* label, bool icon, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImStyle::general_color : hovered ? ImStyle::text::text_hov : ImStyle::text::have_account, g.IO.DeltaTime * 6.f);

    if (icon) PushFont(fonts::icon);
...
bool ImGui::tabs(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImStyle::general_color : hovered ? ImStyle::text::text_hov : ImStyle::text::have_account, g.IO.DeltaTime * 6.f);
...
bool ImGui::SubTabs(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImStyle::general_color : hovered ? ImStyle::text::text_hov : ImStyle::text::have_account, g.IO.DeltaTime * 6.f);
    it_anim->second.rect_size = ImLerp(it_anim->second.rect_size, selected ? 15.f : 0.f, g.IO.DeltaTime * 11.f);

    if (it_anim->second.rect_size > 0.5f) ImGui::GetWindowDrawList()->AddRectFilled(bb1.Min + ImVec2(0, label_size.y + 2), bb1.Max + ImVec2(5 + it_anim->second.rect_size, 27), GetColorU32(ImStyle::general_color), 30);
...
bool ImGui::ChangeList(const char* advertisement, const char* description, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(advertisement);
    const ImVec2 label_size = CalcTextSize(advertisement, NULL, true);
    ...
    static std::map<ImGuiID, change_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, change_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImStyle::changelist::background_active : hovered ? ImStyle::changelist::background_hov : ImStyle::changelist::background, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), ImStyle::button::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(ImStyle::general_color));
    RenderTextClipped(bb.Min + ImVec2(20, 8), bb.Max + ImVec2(20, 8), advertisement, NULL, &label_size);
    PopStyleColor();

    RenderTextClipped(bb.Min + ImVec2(20, 37), bb.Max + ImVec2(20, 37), description, NULL, &label_size);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, advertisement, g.LastItemData.StatusFlags);
    return pressed;
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(ImStyle::button::background_active), style.ScrollbarRounding);
...
void ImGui::ImageRotation(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImU32 tint_col, float speed) {
    ...
    ImGui::SetCursorPos(ImVec2((ImStyle::window::size.x / 2) - size.x / 2, (ImStyle::window::size.y / 2) - size.y / 2));
...
struct check_state {
    ...
    ImVec4 background, checkmark, text;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 25.f;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? label_size.x + 10 : 0.0f), label_size.y));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? ImStyle::general_color : hovered ? ImStyle::checkbox::background_hov : ImStyle::checkbox::background, g.IO.DeltaTime * 6.f);
    it_anim->second.checkmark = ImLerp(it_anim->second.checkmark, *v ? ImStyle::checkbox::checkmark_active : ImStyle::checkbox::checkmark_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImStyle::text::text_active : hovered ? ImStyle::text::text_hov : ImStyle::text::have_account, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), ImStyle::checkbox::rounding);
   if (*v) GetWindowDrawList()->AddShadowRect(check_bb.Min + ImVec2(5, 5), check_bb.Max - ImVec2(5, 5), GetColorU32(it_anim->second.background), 30.f, ImVec2(0, 0), ImStyle::checkbox::rounding);


   RenderCheckMark(window->DrawList, check_bb.Min + ImVec2( (square_sz / 2) - (12 / 2), (square_sz / 2) - (12 / 2) ), GetColorU32(it_anim->second.checkmark), 12.f);

   PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
   if (label_size.x > 0.0f) RenderText(ImVec2(check_bb.Max.x + 10, check_bb.Min.y), label);
   PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImStyle::input::background_active : hovered ? ImStyle::input::background_hov : ImStyle::input::background, g.IO.DeltaTime * 6.f);
        it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() ? 25.f : 15.f, g.IO.DeltaTime * 6.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, size_arg.y - 2), frame_bb.Max, GetColorU32(it_anim->second.background), ImStyle::input::rounding);
        GetWindowDrawList()->AddShadowRect(frame_bb.Min + ImVec2(0, size_arg.y - 2), frame_bb.Max, GetColorU32(it_anim->second.background), it_anim->second.shadow, ImVec2(0, 0), ImStyle::button::rounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(0, 5);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImStyle::input::text_selected); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, ImStyle::input::rounding);
                }
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImStyle::text::have_account : ImStyle::text::text_active);
